...
def two_pointers():
    return ('Метод двух указателей\n'
            ...
            'Мы двигаем первый указатель и двигаем второй указаиель, пока выполняется условие.\n'
            '\n'
            'int i = 0;\n'
            'for (int j = 0; j < n; j++) {\n'
            '    while (i < m && a[i] < b[j]) {\n'
            ...
            '\n'
            'Нам нужно поддерживать минимимум и макимум на двух стеках. Для этого для каждого стека понадобятся ещё два стека: стек минимумов и стека максимумов.\n'
...
def hashes():
    return ('Хэши\n'
            ...
            '  (раздел z_function)\n'
            '\n'
            '# По строке можно построить дерево хэшей (описано в segment_tree)\n'
...
def LIS():
    return ('Наибольшая возрастающая подпоследовательность\n'
            ...
            'Из всех таких ответов возбмём максимальный\n'
            '\n'
            'int n;\n'
            'cin >> n;\n'
            'vector<int> a(n);\n'
...
def ternary_search():
    return ('Тернарный/троичный поиск\n'
            ...
            'Ведь если r - l == 2, то после итерации r и l останутся такими же, либо алгоритм проскочит разницу (l танет больше r)\n'
...
def sqrt_decomposition():
    return ('Корневая декомпозиция\n'
            ...
            'У нас есть 4 варинта треугольника:\n'
            '1) L - L - L\n'
            ...
            'Для вершины L перебём всех соседей\n'
...
def bfs():
    return ('BFS - поиск в ширину\n'
            ...
            '    return 0;\n'
            '}\n'
            'Важно, что bfs ищет длину кратчайшего пути в РЁБРАХ, поэтоу он применяется только на невзвешенных графах\n'
...
def Ford_Bellman():
    return ('Алгоритм Форда-Беллмана\n'
            ...
            'Для этого нужно сделать у массива d второе измерение - кол-во рёбер в пути (ведь кратчайщий путь может состоять и из одного ребра, но нам нужно ровно k рёбер)\n'
            '\n'
            'const int INF = 1e9;\n'
            '\n'
            'struct Edge {\n'
            ...
            'Чтобы восстановить цикл, нужно хранить для каждой вершины, откуда мы в неё пришли, чтобы создать кратчаший путь\n'
            'Тогда будем откатываться по предкам от вершины, для котрой изменилось расстояние, и класть вершины в стек\n'
...
def fractional_cascading():
    return ('Частичное каскадирование\n'
            ...
            '\n'
            'Как найти ответ? Для каждой позиции в каждом массиве предпосчитаем ближайшие элементы из предыдщего массива\n'
...
def cht_opt():
    return ('Convex Hull Trick\n'
            ...
            'Дальше нужно продолжить алгоритм. Т.е. мы пытаемся пересечь новую прямую с прямыми в конце, псотпенно их удаляя\n'
            ...
            '\n'
            'Как найти значение dp[i], если мы построили такую структру?\n'
...
def matrix_opt():
    return ('Матричная оптимизация ДП\n'
            ...
            '0, 1, 0,   0\n'
            '0, 0, 1,   0\n'
            'Числа в матрице определяются коэффициентами в выражении, т.к. c[i][j] - сумма произведений соотвествующих элементов строки первой матрицы и столбца второй матрицы\n'
...
# TODO: разделить geometry (пока делить нечего)
...
# 80% COMPLETE (хотелось бы ещё DCP) Приколы с декомпозицией (всё, что было в ЛКШ)
...
# COMPLETE 9) Convex Hull Trick, матричная оптимизация
...
# MB COMPLETE ~15) Мб, переделать отрицательный цикл
